...
console.log(
  "%c👋 Hej! Widzę, że interesujesz się kodem!",
  "color: #667eea; font-size: 16px; font-weight: bold;"
);
console.log(
  "%c🚀 Szukasz developera? Skontaktuj się z nami: kontakt@design-web.pl",
...
window.addEventListener("load", () => {
  if ("performance" in window && "timing" in window.performance) {
    ...
    if (loadTime > 3000) {
      console.warn(
        "⚠️ Strona ładuje się wolno. Rozważ optymalizację obrazków i skryptów."
      );
    }
  }
});
...
if (contactForm) {
  contactForm.addEventListener("submit", async (e) => {
    ...
    if (formData.message.length < 10) {
      showNotification(
        "Wiadomość jest zbyt krótka (minimum 10 znaków)",
        "error"
      );
      return;
    }
    ...
    if (formData.message.length > 5000) {
      showNotification(
        "Wiadomość jest zbyt długa (maksimum 5000 znaków)",
        "error"
      );
      return;
    }
